...
Sean can simply find the first question he wasn't planning to answer in the list below, and then answer it too.
The table lists all the submitted AMA questions,
sorted based on quantum random numbers generated by the
[ANU QRNG service](https://qrng.anu.edu.au).
As such, every permutation of the question list will exist in at least one branch of the Everettian multiverse.

This table will be [updated]({repo_url}) periodically as new questions are [posted to Patreon]({post_url}).
"""
